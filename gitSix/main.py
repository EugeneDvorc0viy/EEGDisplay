...
import random
import mainOne, mainTwo, mainThree, mainFour
...
class WindowOne(QtWidgets.QMainWindow, mainOne.Ui_WindowCreate):
    ...
    def initUi(self):
        ...
        self.btnBd.clicked.connect(self.goWindowData)
    ...
    @pyqtSlot()
    def goWindowData(self):
        self.window = WindowFour()
        self.window.show()
        self.close()
...
class WindowTwo(QtWidgets.QMainWindow, mainTwo.Ui_MainWindow):
    ...
    def initUi(self):
        ...
        self.btnCreate.clicked.connect(self.goWindowOne)
        self.btnBd.clicked.connect(self.goWindowData)
    ...
    def workTask(self, element):
        ...
        try:
            if self.element:
                ...
                self.btnStart.setText("Start")
                nameTest = self.testNameDialog()
                jsonSave.fileDone(self.leHelpJson.text(), nameTest)
        except Exception:
            self.tbInfo.setText(element)

    def testNameDialog(self):
        text, ok = QtWidgets.QInputDialog.getText(self, "Test Dialog", "Inter somethink: ")
        if ok:
    ...
    def showTime(self):
        ...
        jsonSave.addInfo(context, self.fileName)
...
class WindowFour(QtWidgets.QMainWindow, mainFour.Ui_WindowCreate):
    def __init__(self):
        super().__init__()
        self.setupUi(self)
        self.initUi()
        data = jsonSave.opening()
        for elem in data:
            self.cbData.addItem(elem)


    def initUi(self):
        self.nameTest = ""

        self.btnDateShow.clicked.connect(self.showDate)
        self.cbData.activated[str].connect(self.dataTest)
        self.btnDateShow.setEnabled(False)
        self.btnCreate.clicked.connect(self.goWindowOne)
        self.btnDo.clicked.connect(self.goWindowTwo)

    def dataTest(self, text):
        # self.tbData.setText(str((jsonSave.opening()[text])))
        self.nameTest = text
        self.btnDateShow.setEnabled(True)

    def showDate(self):
        jsonSave.showTestResult(self.nameTest)
